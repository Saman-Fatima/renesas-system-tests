...
#Step_03: Run Ethernet UDP iperf test upload.
iperf3 -c ${server_ip} -u -b 0 > $output_dir/${output_file}_upload.txt

#Step_04: Run Ethernet UDP iperf test download.
iperf3 -c ${server_ip} -u -R -b 0 > $output_dir/${output_file}_download.txt
