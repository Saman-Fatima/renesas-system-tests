...
echo "Codec_dec: h265 FPS: 30fps Resolution: 1920x1080 Channel: 1"
	
#Step02: Create output directory
Board_name=`uname -a | awk -F' ' '{print $2}'`
output_dir=${Board_name}_Video_decoding_result
test_dir=${Board_name}_h265dec_1920x1080_30fps_1
...
echo -n "${Board_name},h265,1920x1080,30fps,1," >> ${output_dir}/${test_dir}/output.csv
...
#Step09: Gst pipeline
sync
#gst-launch-1.0 -r filesrc location=./video_data/h265_YUV420_planar_Main_Profile_Level_4_1920x1080_Progressive_8Mbits_30fps_60seconds.mp4 ! qtdemux ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_1920x1080_30fps_01channel_01.txt &
 
 
#H265 decode file source from ffmpeg tool
gst-launch-1.0 -r filesrc location=./video_data/h265_YUV420_planar_Main_Profile_Level_4_1920x1080_Progressive_8Mbits_30fps_60seconds.mp4 ! qtdemux ! queue ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_1920x1080_30fps_01channel_01.txt &
...
#gst-launch-1.0 -r filesrc location=./video_data/Videotestsrc_h265_YUV420_planar_Main_Profile_Level_4_1920x1080_Progressive_8Mbits_30fps_60seconds.mp4 ! qtdemux ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_1920x1080_30fps_01channel_01.txt &
