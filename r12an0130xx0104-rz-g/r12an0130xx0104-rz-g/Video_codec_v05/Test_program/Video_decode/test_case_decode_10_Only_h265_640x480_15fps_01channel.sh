...
echo -n "${Board_name},h265,640x480,15fps,1," >> ${output_dir}/${test_dir}/output.csv
...
#Step09: Gst pipeline
sync
#gst-launch-1.0 -r filesrc location=./video_data/h265_YUV420_planar_Main_Profile_Level_3_640x480_Progressive_1.5Mbits_15fps_60seconds.mp4 ! qtdemux ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_640x480_15fps_01channel_01.txt &
 
 
#H265 decode file source from ffmpeg tool
gst-launch-1.0 -r filesrc location=./video_data/h265_YUV420_planar_Main_Profile_Level_3_640x480_Progressive_1.5Mbits_15fps_60seconds.mp4 ! qtdemux ! queue ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_640x480_15fps_01channel_01.txt &
...
#gst-launch-1.0 -r filesrc location=./video_data/Videotestsrc_h265_YUV420_planar_Main_Profile_Level_3_640x480_Progressive_1.5Mbits_15fps_60seconds.mp4 ! qtdemux ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_640x480_15fps_01channel_01.txt &
