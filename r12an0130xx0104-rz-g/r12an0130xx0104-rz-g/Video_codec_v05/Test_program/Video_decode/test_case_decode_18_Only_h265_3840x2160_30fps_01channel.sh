...
echo "Codec_dec: h265 FPS: 30fps Resolution: 3840x2160 Channel: 1"
	
#Step02: Create output directory
Board_name=`uname -a | awk -F' ' '{print $2}'`
output_dir=${Board_name}_Video_decoding_result
test_dir=${Board_name}_h265dec_3840x2160_30fps_1
...
echo -n "${Board_name},h265,3840x2160,30fps,1," >> ${output_dir}/${test_dir}/output.csv

#Step05: Camera Initialization
 
#Step04: Gen .h264/.h265 media -> video_data/
#gst-launch-1.0 -e v4l2src num-buffers=1800 device=/dev/video0 io-mode=4 ! video/x-raw,width=none,height=,framerate=30/1 ! vspmfilter dmabuf-use=true ! video/x-raw,width=3840,height=2160,format=NV12,framerate=30/1 ! omxh265enc use-dmabuf=true target-bitrate=14000000 ! video/x-h265,profile=\(string\)main,level=\(string\)5.1 ! h265parse ! qtmux ! filesink location=video_data/h265_YUV420_planar_Main_Profile_Level_5.1_3840x2160_Progressive_14Mbits_30fps_60seconds.mp4
 
#Use videotestsrc
#gst-launch-1.0 -r videotestsrc num-buffers=1800 is-live=true ! video/x-raw,width=3840,height=2160,format=NV12,framerate=30/1 ! omxh265enc target_bitrate=14000000 ! video/x-h265,profile=\(string\)main,level=\(string\)5.1,alignment=au ! h265parse ! qtmux ! queue ! filesink location=video_data/Videotestsrc_h265_YUV420_planar_Main_Profile_Level_5.1_3840x2160_Progressive_14Mbits_30fps_60seconds.mp4 name=v -rp v:sink
...
#Step09: Gst pipeline
sync
#gst-launch-1.0 -r filesrc location=./video_data/h265_YUV420_planar_Main_Profile_Level_5.1_3840x2160_Progressive_14Mbits_30fps_60seconds.mp4 ! qtdemux ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_3840x2160_30fps_01channel_01.txt &
 
 
#H265 decode file source from ffmpeg tool
gst-launch-1.0 -r filesrc location=./video_data/h265_YUV420_planar_Main_Profile_Level_5.1_3840x2160_Progressive_14Mbits_30fps_60seconds.mp4 ! qtdemux ! queue ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_3840x2160_30fps_01channel_01.txt &
 
#use with filesrc location=./video_data/Videotestsrc_h265_YUV420_planar_Main_Profile_Level_5.1_3840x2160_Progressive_14Mbits_30fps_60seconds.mp4
#gst-launch-1.0 -r filesrc location=./video_data/Videotestsrc_h265_YUV420_planar_Main_Profile_Level_5.1_3840x2160_Progressive_14Mbits_30fps_60seconds.mp4 ! qtdemux ! h265parse ! omxh265dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_h265_3840x2160_30fps_01channel_01.txt &
