...
echo "Codec: h264_h264 FPS: 30fps Resolution: 1280x960 Channel: 1"
	
#Step02: Create output directory
Board_name=`uname -a | awk -F' ' '{print $2}'`
output_dir=${Board_name}_Video_codec_result
test_dir=${Board_name}-h264_h264-codec-1280x960-30fps-1
...
echo -n "${Board_name},h264_h264,1280x960,30fps,1," >> ${output_dir}/${test_dir}/output.csv
...
#Use videotestsrc
#gst-launch-1.0 -r videotestsrc num-buffers=1800 is-live=true ! video/x-raw,width=1280,height=960,format=NV12,framerate=30/1 ! omxh264enc target_bitrate=4000000 ! video/x-h264,profile=\(string\)main,level=\(string\)3.1,alignment=au ! h264parse ! qtmux ! queue ! filesink location=video_data/Videotestsrc_h264_Mix_dec_YUV420_planar_Main_Profile_Level_3.1_1280x960_Progressive_4Mbits_30fps_60seconds.mp4 name=v -rp v:sink
...
#Step09: Gst-pipeline
#Video_decode
gst-launch-1.0 -r filesrc location=./video_data/h264_Mix_dec_YUV420_planar_Main_Profile_Level_3.1_1280x960_Progressive_4Mbits_30fps_60seconds.mp4 ! qtdemux ! h264parse ! omxh264dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_Mix_h264_1280x960_30fps_01channel_01.txt &
 
#decode use filesrc ./video_data/Videotestsrc_h264_Mix_dec_YUV420_planar_Main_Profile_Level_3.1_1280x960_Progressive_4Mbits_30fps_60seconds.mp4
#gst-launch-1.0 -r filesrc location=./video_data/Videotestsrc_h264_Mix_dec_YUV420_planar_Main_Profile_Level_3.1_1280x960_Progressive_4Mbits_30fps_60seconds.mp4 ! qtdemux ! h264parse ! omxh264dec ! waylandsink max-lateness=-1 qos=false name=v -rp v:sink > /tmp/log_decode_videotestsrc_Mix_h264_1280x960_30fps_01channel_01.txt &
 
#Video_encode
	gst-launch-1.0 v4l2src num-buffers=1800 device=/dev/video0 io-mode=4 ! video/x-raw,width=1280,height=960,framerate=30/1 ! vspmfilter dmabuf-use=true ! video/x-raw,width=1280,height=960,format=NV12,framerate=30/1 ! omxh264enc use-dmabuf=true target-bitrate=4000000 ! video/x-h264,profile=\(string\)main,level=\(string\)3.1 ! h264parse ! qtmux ! filesink location=video_data/h264_Mix_enc_YUV420_planar_Main_Profile_Level_3.1_1280x960_4Mbits_30fps_60seconds.mp4 name=v -rp v:sink > /tmp/log_encode_Mix_h264_1280x960_30fps_01channel_01.txt &
 
#encode use videotestsrc
#gst-launch-1.0 -r videotestsrc num-buffers=1800 is-live=true ! video/x-raw,width=1280,height=960,format=NV12,framerate=30/1 ! omxh264enc target_bitrate=4000000 ! video/x-h264,profile=\(string\)main,level=\(string\)3.1,alignment=au ! queue ! filesink location=video_data/Videotestsrc_h264_Mix_enc_YUV420_planar_Main_Profile_Level_3.1_1280x960_4Mbits_30fps_60seconds.mp4 name=v -rp v:sink > /tmp/log_encode_videotestsrc_Mix_h264_1280x960_30fps_01channel_01.txt &
